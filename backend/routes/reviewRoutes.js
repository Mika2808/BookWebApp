...
router.post('/:bookId/reviews', isNotAdmin, verifyToken, reviewsController.createReview);

// Route to get all reviews for a specific book
router.get('/:bookId/reviews', verifyToken, reviewsController.getReviewsForBook);

// Route to update a specific review (only for the user who created it)
router.put('/:bookId/reviews/:reviewId', isSelf, verifyToken, reviewsController.updateReview);
